...
def stim_annot(ax, stim,
            ...
            loc=(0.29,1.05)):
    title = stim[1::2]+' '+'\n'+stim[::2]
    ax.annotate(title, loc, ha=ha, fontsize=6,
                xycoords=xycoords,
...
data.build_facemotion()
facemotion = gaussian_filter1d(data.facemotion, 10)
...
def prepare_full_view(data):
    ...
    data.build_LFP()
    data.build_facemotion()
    facemotion = gaussian_filter1d(data.facemotion, 40)
...
if 1:
    ...
    for i, key, ax in zip(range(len(all_stimKeys)), all_stimKeys, pt.flatten(AX)):
        inset = pt.inset(ax, (0.3,1.,0.6,0.7)) 
        ...
        if d is not None:
            ax.plot(\
                gaussian_filter1d(\
                    d[mod][:,20:,:].mean(axis=(0,1)),20))
            run = (d['running']>0.1)
            ax.plot(\
                gaussian_filter1d(\
                    d[mod][run,20:,:].mean(axis=(0,1)),50), lw=0.5)
            ax.plot(\
                gaussian_filter1d(\
                    d[mod][~run,20:,:].mean(axis=(0,1)),50), lw=0.5)
            ax.axis('off')

    pt.set_common_ylims(AX[:32])

    for ax in pt.flatten(AX[:32]):
        pt.draw_bar_scales(ax, Xbar=1, 
                        Xbar_label='1s' if ax==AX[0] else '',
                        Ybar=0.05,
                        Ybar_label='5Hz' if ax==AX[0] else '')
# %%

d = load_epData(dataset['files'][0], all_stimKeys[0])
np.sum(d['running']>0.1)
# %%
if 0:
