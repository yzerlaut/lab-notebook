...
Model = {
    ...
    'dt':0.1, 'tstop': 800., 'SEED':3, # low by default, see later
}
...
for model in ['classical', 'updated']:
    ...
    context_cond = (t_array>t0) & (t_array<t1)
    ...
    Model['Fexc_SstInh'][~context_cond] += 50.
    if model=='updated':
        Model['Fexc_ChInh'][context_cond] += 80.
        Model['Fexc_PyrExc'][context_cond] -= 10.

    stim_cond = (t_array>t0) & (t_array<t1)

    Model['Fexc_PyrExc'][stim_cond] += 90
...
for model in ['classical', 'updated']:
    ...
    fig, AX = ntwk.plots.pretty(data, 
                    ...
                    Vm_args=dict(subsampling=1, lw=0.5, shift=40.,
                                clip_spikes=False, vpeak=0),
            fig_args=dict(ax_scale=(1.5, 1.), dpi=150,
                            hspace=0.1, bottom=0.2, top=0.2, 
                            left=0.1, right = 0.1))
    ylim = AX[-1].get_ylim()
    AX[-1].plot([t0, t1], (ylim[1]-4)*np.ones(2), '-', color='lightgrey', lw=2)
    AX[-1].plot([t0, t1], (ylim[1]-3)*np.ones(2), 'k-', lw=1)
