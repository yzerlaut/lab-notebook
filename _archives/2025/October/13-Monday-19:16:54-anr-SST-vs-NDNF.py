...
SST_DATASET = \
    physion.analysis.read_NWB.scan_folder_for_NWBfiles(
        os.path.expanduser('~/DATA/Cibele/SST-Adult/NWBs'),
...
def mesh_plot(ep, cond, ax, lim=5):
    zScore = ep.dFoF[cond,:,:].mean(axis=0).T
    zScore -= zScore[ep.t<0,:].mean(axis=0)
    zScore /= zScore[:,:].std(axis=0)
    print(lim)
    plt.subplots_adjust(wspace=1.)
    im = ax.pcolormesh(ep.t,
    ...
    # ax.axvline( df_vis.iloc[18]['duration'], color='black', linestyle='--')
    ax.set(xlabel='time (s)', ylabel='neurons', 
           xticks=[0,2,4])

# %%
for f in SST_DATASET['files'][2:3]:
    data = physion.analysis.read_NWB.Data(f)
    data.build_dFoF(smoothing=4)
    ep = physion.analysis.process_NWB.EpisodeData(data,
                                                  quantities=['dFoF'],
                                                  prestim_duration=3)
    
    fig, axs = plt.subplots(1, 2, figsize=(6, 2))
    cond = ep.find_episode_cond(key=['contrast', 'angle'], index=[1,1])
    mesh_plot(ep, cond, axs[0])
    cond = ep.find_episode_cond(key=['contrast', 'angle'], index=[6,1])
    mesh_plot(ep, cond, axs[1])
    

# %%

# %%
data = physion.analysis.read_NWB.Data('/Users/yann/DATA/Cibele/SST-Adult/NWBs/2024_08_27-11-16-53.nwb')
data.init_visual_stim(degree=True)

ax = data.visual_stim.plot_stim_picture(3, with_scale=20)
ax = data.visual_stim.plot_stim_picture(7, with_scale=20)

# %%
i1 = np.arange(
        len(data.visual_stim.experiment['contrast']))[np.array(data.visual_stim.experiment['contrast'])==1][0]
print(i1)
# %%
