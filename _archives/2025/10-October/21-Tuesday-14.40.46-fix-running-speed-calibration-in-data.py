        ...
        if os.path.isfile(os.path.join(d, 'metadata.json')):
            ...
            for key in text_replace:
                ...
                if len(content.split(key))>1:
                    print('   - replacing value in: %s ' % os.path.join(d, 'metadata.json'))

                if False: # for now, for debugging
                    content.replace(key, text_replace[key])
            ...
            if metadata["roto-encoder-value-per-rotation"]==-25300.0:
                print('   - replacing value in: %s ' % os.path.join(d, 'metadata.npy'))
