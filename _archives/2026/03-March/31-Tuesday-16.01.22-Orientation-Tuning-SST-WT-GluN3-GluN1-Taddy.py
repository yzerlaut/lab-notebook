...
for i, case in enumerate(cases):
    Widths[case] = []
    c1 = [np.mean(r, axis=0) for r in SUMMARY[case]['WAVEFORMS_c=1.0'] if len(np.array(r).shape)==2]
    for c, contrast in enumerate([0.5, 1.0]):

        resp = [np.mean(r, axis=0) for r in SUMMARY[case]['WAVEFORMS_c=%.1f' % contrast] if len(np.array(r).shape)==2]
        ...
        if c:
            pt.annotate(inset,'N=%i sess.' % len(SUMMARY[case]['WAVEFORMS_c=%.1f' % contrast]) + i*'\n',
                        (1, 1.2), ha='right', color=colors[i])
...
iFig +=1
fig.savefig(os.path.join(Fig_Folder, 'OS-%i.svg' % iFig))
...
def generate_comparison_figs(SUMMARY, cases, contrasts,
    ...
    fig, [ax, ax2,inset] = plt.subplots(1, 3, figsize=(6, 1))
    plt.subplots_adjust(top=0.9, bottom=0.2, wspace=0.4)
    #inset = pt.inset(ax2, (1.7, 0.2, 0.3, 0.8))

    SIs = []
    for i, key, contrast in zip(range(3), cases, contrasts):

        if average_by=='sessions':
            resp = np.array([np.mean(r, axis=0)\
                             for r in SUMMARY[key]['RESPONSES_c=%.1f' % contrast] if len(np.array(r).shape)==2])
        else:
        ...
        print(key, 'c=%.1f' % contrast, 'average by:', average_by, 
              ', OSI= %.2f +/- %.2f' % (np.mean(SIs[-1]), stats.sem(SIs[-1])))
        try:
            if average_by=='sessions':
    ...
    inset.plot([0.05,0.95], 1.05*np.ones(2), 'k-', lw=0.5)
    inset.annotate(pt.from_pval_to_star(stats.mannwhitneyu(SIs[0], SIs[1]).pvalue),
                   (0.5, 1.09), fontsize=6, ha='center')
    inset.plot([1.05,1.95], 1.05*np.ones(2), 'k-', lw=0.5)
    inset.annotate(pt.from_pval_to_star(stats.mannwhitneyu(SIs[1], SIs[2]).pvalue),
                   (1.5, 1.09), fontsize=6, ha='center')

    pt.set_plot(inset, xticks=[], ylabel='select. index', yticks=[0, 0.5, 1], ylim=[0, 1.09], xlim=[-2,4])
...
fig1, ax = generate_comparison_figs(SUMMARY, CASES, [1., 1., 1.],
                                    average_by='sessions', norm='norm. ', colors=COLORS)
pt.annotate(ax, ' contrast = 1, average by sessions', (0.5,1.2), ha='center')


fig2, ax = generate_comparison_figs(SUMMARY, CASES, [1., 1., 1.],
                                    average_by='ROIs', norm='norm. ', colors=COLORS)
pt.annotate(ax, ' contrast = 1, average by ROIs', (0.5,1.2), ha='center')

fig3, ax = generate_comparison_figs(SUMMARY, CASES, [0.5, 0.5, 0.5],
                                    average_by='sessions', norm='norm. ', colors=COLORS)
pt.annotate(ax, ' contrast = 0.5, average by sessions', (0.5,1.2), ha='center')


fig4, ax = generate_comparison_figs(SUMMARY, CASES, [0.5, 0.5, 0.5],
                                    average_by='ROIs', norm='norm. ', colors=COLORS)
pt.annotate(ax, ' contrast = 0.5, average by ROIs', (0.5,1.2), ha='center')

for fig in [fig1, fig2, fig3, fig4]:
    iFig +=1
    fig.savefig(os.path.join(Fig_Folder, 'OS-%i.svg' % iFig))
    
#fig2.suptitle(' contrast = 1, average by ROIs')
