...
def my_cwt(data, frequencies, dt, w0=6.):
    ...
    """
    output = np.zeros([len(frequencies), len(data)], dtype=np.complex64)
...
def compute_freq_envelope(signal, sampling_freq, freqs):
    ...
    """
    return np.max(np.abs(cwt(signal,
                             freqs,
                             1./sampling_freq)),
                             axis=0)

fig, ax = plt.subplots()
ax.plot(t, compute_freq_envelope(np.random.randn(10,len(t))[0,:], 1./dt, freqs))
