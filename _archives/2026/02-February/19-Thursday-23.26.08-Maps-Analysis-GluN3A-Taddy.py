...
JICT = 1 # joint_ipsi_and_contra_threshold, set to None to desactivate !
SUFFIX= '-2s-respThreshold-1'
...
def preprocess_maps(d, 
                    ...
                    joint_ipsi_and_contra_threshold=JICT,
                    smoothing=4):
    ...
    for loc in ['Pre', 'Post']:
        ...
        if joint_ipsi_and_contra_threshold is not None:
            cond = (d['%s-%s_dFoF' % (loc, 'Ipsi')]>\
                        joint_ipsi_and_contra_threshold) &\
                    (d['%s-%s_dFoF' % (loc, 'Ipsi')]>\
                        joint_ipsi_and_contra_threshold)
            for k, key in enumerate(['Ipsi', 'Contra', 'Blank']):
                d['%s-%s_dFoF' % (loc, key)][~cond] = np.nan
...
def plot_mouse(d, window, label='', 
    ...
    for l, loc in enumerate(['Pre', 'Post']):
        ...
        bounds = [\
            1.1*np.nanmin([d['%s-%s_%s' % (loc, key, Map)]\
                    for key in ['Ipsi', 'Contra', 'Blank']]),
            1.0*np.nanmax([d['%s-%s_%s' % (loc, key, Map)]\
                    for key in ['Ipsi', 'Contra', 'Blank']])
        ]
...
data = load_mouse(0) # change below:
...
for t, typ in enumerate(['map', 'baseline', 'dResp', 'dFoF']):
    bounds = [\
        1.1*np.nanmin([data['%s-%s_%s' % (loc, key, typ)]\
                for key in ['Ipsi', 'Contra', 'Blank']]),
        1.0*np.nanmax([data['%s-%s_%s' % (loc, key, typ)]\
                for key in ['Ipsi', 'Contra', 'Blank']])
    ]
    for k, key in enumerate(['Ipsi', 'Contra', 'Blank']):
    ...
    for k, key in enumerate(['Ipsi', 'Contra', 'Blank']):
        ...
        for id in IDS:
            ...
            AX[k].plot([2*i-.1, 2*i+1+.2],
                [np.nanmean(d['%s-%s_dFoF' % (loc, key)]) for loc in ['Pre', 'Post']],
                '-', color=color, lw=0.5)

            for l,  loc in enumerate(['Pre', 'Post']):
                final[label][loc][key].append(\
                            np.nanmean(d['%s-%s_dFoF' % (loc, key)]))
