...
def compute_mean_map(t, Resp,
                     response_window = [0.2,0.5],
                     smoothing=0.1):

    # F0 = np.percentile(Resp[t<0,:,:], F0_percentile, axis=0)
    F0 = np.mean(Resp[t<0,:,:][1:,:,:], axis=0)
    #
        ...
        for loc in ['Pre']:
            for key in ['Ipsi', 'Contra', 'Blank']:
                d['%s-%s_map' % (loc, key)] = \
                    ...
                    compute_mean_map(d['t'], d['%s-%s' % (loc, key)],
                                     response_window=[-0.2,-0.0])

        fig = plot_mouse(d, label=label, bounds=[0, 0.4])
        fig.savefig(os.path.join(tempfile.gettempdir(), '%i.png' %n), 
                    transparent=False)
        n+=1

# %%
fig.savefig(os.path.expanduser('~/Desktop/test.svg'))
# %%
